...
const MakerspaceChat = ({ makerspaces = [] }) => {
  ...
  const [messages, setMessages] = useState([
    {
      type: 'bot',
      content: '👋 Hi! I can help you with makerspace questions like "Create me a lesson plan using sewing machines and a Toronto makerspace, for kids" or "Where can I learn 3D printing in Toronto?"'
    }
  ]);
  ...
  return (
    <>
      ...
      {/* Chat Window */}
      {isOpen && (
        <div className={`fixed bottom-24 right-6 bg-white rounded-3xl shadow-2xl border border-gray-100 z-40 flex flex-col overflow-hidden transition-all duration-300 ${
          isExpanded 
          ...
          <div className="p-3 border-b border-gray-100 bg-gray-100 shadow-sm z-10 flex items-center justify-between">
            <h3 className="text-m font-medium text-gray-900">Ask Me Anything About Makerspaces</h3>
            <button
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50/30">
            {messages.map((message, index) => (
              <div
                key={index}
                className={`flex text-left ${message.type === 'user' && 'justify-end'}`}
              >
                <div
